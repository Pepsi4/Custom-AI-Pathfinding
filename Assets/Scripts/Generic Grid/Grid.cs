...
public class Grid<TGridObject>
{
    ...
    {
        _debugTextArray = new TMPro.TextMeshPro[_width, _height];

        for (int x = 0; x < _gridArray.GetLength(0); x++)
        {
            for (int y = 0; y < _gridArray.GetLength(1); y++)
            {
                _debugTextArray[x, y] = UtilsClass.CreateWorldText(_gridArray[x, y]?.ToString(), _parent, GetWorldPosition(x, y) + new Vector3(_cellSize, _cellSize) * .5f, _cellSize * 4, Color.white, TextAnchor.MiddleCenter);
